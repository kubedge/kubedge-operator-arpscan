...
generate: setup
	...
	go run vendor/k8s.io/code-generator/cmd/deepcopy-gen/main.go --input-dirs github.com/kubedge/kubedge-operator-arpscan/pkg/apis/kubedgeoperators/v1alpha1 -O zz_generated.deepcopy --bounding-dirs github.com/kubedge/kubedge-operator-arpscan/pkg/apis
...
docker-build-amd64:
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o build/_output/amd64/kubedge-arpscan-operator -gcflags all=-trimpath=${GOPATH} -asmflags all=-trimpath=${GOPATH} -tags=v1 ./cmd/...
	docker build . -f build/Dockerfile.amd64 -t ${IMG_AMD64}
	docker tag ${IMG_AMD64} ${DHUBREPO_AMD64}:latest

docker-build-arm32v7:
	GOOS=linux GOARM=7 GOARCH=arm CGO_ENABLED=0 go build -o build/_output/arm32v7/kubedge-arpscan-operator -gcflags all=-trimpath=${GOPATH} -asmflags all=-trimpath=${GOPATH} -tags=v1 ./cmd/...
	docker build . -f build/Dockerfile.arm32v7 -t ${IMG_ARM32V7}
	docker tag ${IMG_ARM32V7} ${DHUBREPO_ARM32V7}:latest

docker-build-arm64v8:
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -o build/_output/arm64v8/kubedge-arpscan-operator -gcflags all=-trimpath=${GOPATH} -asmflags all=-trimpath=${GOPATH} -tags=v1 ./cmd/...
...
install-dev: docker-build-dev
	helm install --name kubedge-arpscan-operator chart --set images.tags.operator=${IMG_DEV}

install-amd64:
	helm install --name kubedge-arpscan-operator chart --set images.tags.operator=${IMG_AMD64}

install-arm32v7:
	helm install --name kubedge-arpscan-operator chart --set images.tags.operator=${IMG_ARM32V7}

install-arm64v8:
	helm install --name kubedge-arpscan-operator chart --set images.tags.operator=${IMG_ARM64V8}

purge: setup
	helm delete --purge kubedge-arpscan-operator
